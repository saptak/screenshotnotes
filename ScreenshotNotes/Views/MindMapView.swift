...
struct MindMapView: View {
    ...
    var body: some View {
        NavigationStack {
            ...
            .fullScreenCover(isPresented: $showingNodeDetail) {
                if let selectedScreenshot = selectedScreenshot {
                    ScreenshotDetailView(
                        screenshot: selectedScreenshot,
                        heroNamespace: heroNamespace,
                        allScreenshots: getFilteredScreenshots(),
                        onDelete: nil
                    )
                }
            }
    ...
    private func getFilteredScreenshots() -> [Screenshot] {
        // Get screenshots that have semantic data and are represented in the mind map
        let nodeArray = mindMapService.mindMapData.nodeArray
        let nodeScreenshotIds = Set(nodeArray.map { $0.screenshotId })
        
        return screenshots.filter { screenshot in
            nodeScreenshotIds.contains(screenshot.id)
        }
    }
}
...
// NodeDetailView removed - using unified ScreenshotDetailView instead
